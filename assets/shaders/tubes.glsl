...
// this shader is an adjusted version of https://www.shadertoy.com/view/lsc3DH
...
void mainImage(out vec4 fragColor, in vec2 fragCoord) {
    ...
    vec3 rd = normalize(vec3(2. * fragCoord - u_resolution.xy, u_resolution.y));

    // time factor
    float t_factor = .5;

    // Rotating the ray with Fabrice's cost cuttting matrix. I'm still pretty happy with this also. :)
    vec2 m = sin(vec2(1.57079632, 0) + t_factor * u_time / 4.);
    ...
    vec3 ro = vec3(0.0, t_factor * u_time / 2. + 0.5, t_factor * u_time / 2.);
    ...
    vec3 oCol = tex3D(u_texture_0, sp, sn); // Texture color at the surface point.
    ...
    vec3 rCol = tex3D(u_texture_0, rsp, rsn); // Texel at "rsp."
...
void main() {
    mainImage(gl_FragColor, gl_FragCoord.xy * 1.);
}
